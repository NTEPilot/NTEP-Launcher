...
use crate::{
    ...
    setup::{cleanup_runtime_for_rebuild, setup_environment, setup_workspace, SplashUpdate},
};
...
const BACKEND_ERROR_URL_BASE: &str = "http://ntep-error.localhost/backend";
#[cfg(not(any(windows, target_os = "android")))]
const BACKEND_ERROR_URL_BASE: &str = "ntep-error://localhost/backend";
#[cfg(any(windows, target_os = "android"))]
const SPLASH_URL: &str = "http://ntep-splash.localhost/";
#[cfg(not(any(windows, target_os = "android")))]
const SPLASH_URL: &str = "ntep-splash://localhost/";
...
fn time_bomb_config() -> Result<Option<TimeBombConfig>> {
    let Some(section) = cargo_toml_section("package.metadata.ntep-launcher.time-bomb") else {
        return Ok(None);
    };
...
mod tests {
    ...
    fn test_time_bomb_config_parses_when_enabled() {
        let section =
            cargo_toml_section("package.metadata.ntep-launcher.time-bomb").expect("section exists");
    ...
    fn test_english_splash_i18n_uses_json_literals() {
        ...
        assert!(html.contains(r#""defaultTip":"NTEP Launcher is starting the local service."#));
        assert!(!html.contains("const defaultTip = '"));
        assert!(html.contains("window.__NTEP_SPLASH_READY = true;"));
        let old_prefix = ['A', 'L', 'A', 'S'].into_iter().collect::<String>();
        let old_splash_ready_global = format!("__{old_prefix}_SPLASH_READY");
        assert!(!html.contains(&old_splash_ready_global));
    }
}
...
fn main() -> Result<()> {
    ...
    let webui_config = WebuiLaunchConfig::default();
    ...
    tauri::Builder::default()
        .register_uri_scheme_protocol("ntep-error", |_ctx, request| {
            backend_error_response(request)
        })
        .register_uri_scheme_protocol("ntep-splash", |_ctx, _request| splash_response())
            ...
            match event {
                tauri::RunEvent::Ready => {
                    ...
                    thread::spawn(move || {
                        ...
                        if let Err(e) =
                            setup_workspace(&mut status_updater, setup_cancel_requested.clone())
                        {
                            error!("{e}");
                            setup_running.store(false, Ordering::SeqCst);
                            ...
                            status_updater(SplashUpdate::error(
                                t!("dialog.startup_failed"),
                                t!("dialog.workspace_setup_failed", error = e.to_string()),
                                last_progress.get().max(8),
                            ));
                            return;
                        }
                        info!("Starting main.py on http://127.0.0.1:{}/", port);
                        status_updater(
                            SplashUpdate::loading(
                                t!("splash.starting"),
...
fn page_load_injector(webview: WebviewWindow, payload: PageLoadPayload<'_>) {
    if payload.event() == PageLoadEvent::Finished {
        ...
        let injected_js = r#"
if (!window.ntep_launcher_injected) {
    window.ntep_launcher_injected = true;
    (function () {
        // Prevent going back
...
fn update_splash(splash: &WebviewWindow, update: &SplashUpdate) {
    ...
    let script = format!("window.__NTEP_SPLASH_UPDATE && window.__NTEP_SPLASH_UPDATE({payload});");
...
<body>
  <div class="launcher-window">
    <div id="splash-drag-region" class="top-bar">
      <div class="brand-zone">
        <span class="app-title">NTEP Launcher</span>
  ...
  <script>
    ...
    window.__NTEP_SPLASH_UPDATE = function (payload) {
    ...
    window.__NTEP_SPLASH_READY = true;
  </script>
</body>
</html>"#
