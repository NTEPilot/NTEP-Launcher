...
#[cfg(windows)]
const WINDOWS_APP_ID: &str = "app.ntepilot.launcher.notification";

#[cfg(windows)]
const WINDOWS_APP_ID_UPDATE: &str = "app.ntepilot.launcher.notification.update";

#[cfg(windows)]
const WINDOWS_APP_ID_ANNOUNCEMENT: &str = "app.ntepilot.launcher.notification.announcement";
...
mod tests {
    ...
    fn parses_notify_payload() {
        let payload: NotifyPayload = serde_json::from_str(
            r#"{"instance":"ntep","title":"NTEP Launcher <ntep> 警告","content":"<ntep> 任务提醒"}"#,
        )
        .unwrap();

        assert_eq!(payload.instance.as_deref(), Some("ntep"));
        assert_eq!(payload.title.as_deref(), Some("NTEP Launcher <ntep> 警告"));
        assert_eq!(payload.content.as_deref(), Some("<ntep> 任务提醒"));
    }
}
